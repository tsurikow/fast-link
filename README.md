...
Comprehensive unit and functional tests for Fast-Link have been implemented using the following tools:

- **Coverage**: Provides test coverage of 94% of the code.
- **HTTPX**: Used for making asynchronous HTTP requests during tests.
- **Pytest**: The primary testing framework.
- **Pytest-asyncio**: Supports asynchronous tests.
- **Pytest-html**: Generates HTML reports for test results.
- **Pytest-mock**: Facilitates mocking in tests.

A total of 36 tests verify the core functionalities and edge cases of the application.

In addition to unit and functional tests, load testing has been performed using **Locust**. All test reports, including those generated by Locust, are available in HTML format in the `reports` folder.

> **Note:** The Locust service is configured to start only after all tests run successfully. Once started, it is accessible at [http://0.0.0.0:8089/](http://0.0.0.0:8089/).

### How to Run Tests

To run all tests and generate the reports, execute the following command:
